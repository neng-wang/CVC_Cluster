...
std::vector<Bbox> Cluster(const std::vector<Eigen::Vector3d>& frame,
    ...
    for (size_t i = 0; i < frame.size(); i++) {
           if (
               ...
               frame_label[i] == 18 
                //   frame_label[i] == 19
            //    frame_label[i] == 15 ||  // vegetration
            //    frame_label[i] == 14 ||  // fence
            //    frame_label[i] == 13   // build
            //    frame_label[i] == 17  // terrian
               ) {  // 只聚类感兴趣的目标  //pole
    ...
    for (size_t i = 0; i < 260; i++) {
           if (pc_semantic[i]->empty())
                continue;
            // if(i==1){
            //     std::string out_file1 = "./match_node_inGraph1.txt";
            //     std::ofstream f_out1(out_file1);
            //     std::cout<<"match node size:"<<pc_semantic[i]->size()<<std::endl;
            //     for(int j=0;j<pc_semantic[i]->size();j++){
            //         f_out1<<(*pc_semantic[i])[j].x()<< " "<<(*pc_semantic[i])[j].y()<< " "<<(*pc_semantic[i])[j].z()<<std::endl;
            //     }
            //     f_out1.close();
            // }
           ...
           for (size_t m = 0; m < tr.size(); m++) {
                ...
                if(i==1||i==4||i==5){
                    ...
                    // if(tr[m].second.num < 10) continue;
                    bbox_cluster.label = 4; //表示build
                }
                // else if(i==19){
                //     // if(tr[m].second.num < 10) continue;
                //     bbox_cluster.label = 1; //表示build
                // }
                ...
                // // distance check
                // if(bbox_cluster.center[0]<-50 || bbox_cluster.center[0] >50 || bbox_cluster.center[1]<-50 || bbox_cluster.center[1]>50)
                //     continue;
                ...
                if(bbox_cluster.label ==1){
                    if(bbox_cluster.dimension[0]<0.5 || bbox_cluster.dimension[1]<0.5 || bbox_cluster.dimension[2]<0.3 )
                        continue;
                    // if(bbox_cluster.dimension[0]>1 || bbox_cluster.dimension[1]>1 || bbox_cluster.dimension[2]>1)
                    //     continue;
                }
                else if (bbox_cluster.label == 2 || bbox_cluster.label == 3)
                {
                    if(bbox_cluster.dimension[2]<0.5){
                        continue;
                    }
                }
                else if (bbox_cluster.label == 4 )
                {
                    if((bbox_cluster.dimension[0]*bbox_cluster.dimension[1]*bbox_cluster.dimension[2])<5){
                        continue;
                    }
                }
